...
exports.contents = (repo, contents, analyzeTag, cb) => {
  ...
  if (opt.isTag) {
    ...
    // });
    if (analyzeTag) {
      ghtags.list(remote.file()[opt.owner].token, opt.owner, opt.repo, tags => {
        try  {
          opt.targetVersion = version.analyze(tags, opt.versionRange, opt.owner, opt.repo, () => tags[0]);
        } catch(e) {
          throw Error("Failed analyzing tags of " + opt.owner + "/" + opt.repo + ": " + e);
        }
        _contents(ghrepo, opt, contents, cb)
      })
    } else {
