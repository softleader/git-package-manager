...
exports.setVariables = (owner, repo, tag) => {
  variables = {"${owner}": owner, "${repo}": repo, "${tag}": tag};
};

exports.getVariableValue = (originValue) => {
  const variableValue = variables[originValue];
  return variableValue ? variableValue : originValue;
};
