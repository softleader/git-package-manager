...
const getVersion = (versions, versionRange) => {
  let findRegx, extension;
  if("*" === versionRange) {
    findRegx = "(.RELEASE)$";
    extension = ".RELEASE";
  } else {
    const n = versionRange.lastIndexOf(".");
    extension = n === -1 ? "" : versionRange.substring(n + 1);
    extension = isNaN(extension) ? "." + extension : "";
    findRegx = "" === extension ? "." : "(" + extension + ")$";
  }

  // console.log(semver.maxSatisfying(["v1.0.0","v1.0.1","v2.0.4","v2.0.2"], "*"));
  versions = versions.filter(version => version.match(findRegx)).map(version => formatVersion(version, findRegx));
  // console.log(versions);

  if(versions && versions.length > 0) {
    const targetVersion = semver.maxSatisfying(versions, formatVersion(versionRange, findRegx))  + extension;
    console.log("find matched version: %s", targetVersion);
    return targetVersion;
  }

  console.log("no matched 'RELEASE or RC' tag");
  return "no 'RELEASE or RC' tag";
};

const formatVersion = (version, replaceRegx) => {
  return "." === replaceRegx ? version : version.replace(new RegExp(replaceRegx,"gm"), "");
};
