...
const installDependency = (githubRepo) => {
  ...
  Git.Clone(url, clonePath, opts(github)).done(repository => {
    Git.Tag.list(repository).done(tags => {
      const version = getVersion(tags, versionRange);
      // repository.getBranch(version).done(reference => {
      //   console.log(reference);
      //   repository.checkoutRef(reference).done(() => {
      //     updateYaml(githubRepoNoVersion, yamlJson, "^" + version);
      //   });
      // });
      // repository.checkoutBranch(version, opts).done(() => {
      //   updateYaml(githubRepoNoVersion, yamlJson, "^" + version);
      // });
      // Git.Tag.lookup(repository, version).done(tag => {
      //   console.log(tag);
      //   Git.Checkout.tree(repository, tag.targetId(), { checkoutStrategy: Git.Checkout.STRATEGY.SAFE_CREATE}).done(() => {
      //     repository.setHeadDetached(tag.targetId(), repository.defaultSignature, "Checkout: HEAD " + tag.targetId());
      //     updateYaml(githubRepoNoVersion, yamlJson, "^" + version);
      //   });
      // });
      updateYaml(githubRepoNoVersion, yamlJson, "^" + version);
...
const getVersion = (versions, versionRange) => {
  ...
  versions = versions.filter(version => version.match(findRegx)).map(version => {
    return {
      origin: version,
      format: version.replace(new RegExp(findRegx,"gm"), "")
    }
  });

  if(versions && versions.length > 0) {
    const targetVersion = semver.maxSatisfying(
      versions.map(version => version.format),
      versionRange.replace(new RegExp(findRegx,"gm"), ""));
    return versions.find(version => version.format === targetVersion).origin;
  }
