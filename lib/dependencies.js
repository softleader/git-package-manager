...
exports.install = (repos, options) => {
  ...
  if (options.yamlFile.startsWith('github:')) {
    ...
    const version = splited.length > 1 ? splited.slice(1).join('#') : 'master';
...
const cloneGithub = (url, clonePath, ownerRepo, options) => {
  ...
  Git.Clone(url, clonePath, opts(owner)).done(repository => {
    ...
    if(repositoryMapping[ownerRepo].isTag) {
      ...
      console.log(format("Receiving branch '{}/{}#{}'", owner, repo, versionRange));
      repository.getBranch('refs/remotes/origin/' + versionRange)
        .done(reference => {
          repository.checkoutRef(reference);
          updateYaml(format("{}/{}", owner, repo), "#" + versionRange, options);
        });
    }
  });
};
  ...
  ghrepo.tags((error, tags, header) => {
    if (error) {
      ...
      if(repositoryMapping[ownerRepo].isTag) {
        ...
        version = getVersion(tags, versionRange, owner, repo, () => tags[0]);
      } else {
        console.log(format("Receiving branch '{}/{}#{}'", owner, repo, versionRange));
...
const getVersion = (versions, versionRange, owner, repo, latestVersionSupplier) => {
  const matchVersionMessage = format("Receiving tag '{}/{}@{}' with version: {}", owner, repo, versionRange, "%s");
