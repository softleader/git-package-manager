...
exports.install = (repos, options) => {
  ...
  if (options.yamlFile.startsWith('git:')) {
    const result = /^git:(.+?)\/(.+?)\/(.+)/g.exec(options.yamlFile);
...
const parseRepository = (ownerRepo) => {
  ...
  repositoryMapping[ownerRepo].owner = ownerRepo.includes("@") ? result[1] : result[4];
  repositoryMapping[ownerRepo].repo = ownerRepo.includes("@") ? result[2] : result[5];
  repositoryMapping[ownerRepo].versionRange = ownerRepo.includes("@") ? result[3] : "*";
};
