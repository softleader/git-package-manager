...
const _install = (repos, options) => {
  ...
  if(repos.length > 0) { // gpm install owner/repository
    ...
    dependencies.map(dependency => {
      const key = Object.keys(dependency.repository)[0];
      repositoryGroup[key] = dependency.group;
      return key + "@" + dependency.repository[key];
...
const installDependency = (ownerRepo, options) => {
  ...
  const group = options.group ? options.group : repositoryGroup[ownerRepo];
