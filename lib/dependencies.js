...
const installDependency = (ownerRepo, options) => {
  ...
  if(options.contents.length > 0) {
    cloneGithubWithSpecifyFile(clonePath, owner, repo, versionRange, options.contents);
  } else {
...
const cloneGithubWithSpecifyFile = (clonePath, owner, repo, versionRange, contents) => {
        ...
        request({
          ...
          if(error || response.statusCode !== 200) {
            ...
            fs.outputFile(outputFile, bodyJson.content, bodyJson.encoding, () => {
              const yamlJson = getYamlJson();
              if(yamlJson.filtering) { // begin string replace
