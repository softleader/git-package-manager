...
const cloneGithubWithSpecifyFile = (clonePath, user, repo, versionRange, contents) => {
  ...
  request({
    ...
    if(error || response.statusCode !== 200) {
      console.error("get tags error, status: %s, message: %s", response.statusCode, error ? error.message : response.statusMessage);
    } else {
      ...
      contents.forEach(content => {
        request({
          ...
          if(error || response.statusCode !== 200) {
            console.error("get file error, status: %s, message: %s",
              response.statusCode, error ? error.message : response.statusMessage);
          } else {
            updateYaml(format("{}/{}", user, repo), matchAllVersionRange(versionRange) ? "^" + version : versionRange);
