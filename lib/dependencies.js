...
const installDependency = (githubRepo) => {
  ...
  Git.Clone(url, clonePath, opts(github)).done(repository => {
    Git.Tag.list(repository).done(tags => {
      ...
      //   updateYaml(githubRepoNoVersion, yamlJson, "^" + version);
      // });
      // repository.getTagByName(version).done(tag => {
      //   console.log(tag.targetId());
      //   repository.checkoutBranch(tag.targetId(), opts).done(() => {
      //     updateYaml(githubRepoNoVersion, yamlJson, "^" + version);
      //   });
...
const getVersion = (versions, versionRange) => {
  ...
  const extension = n === -1 ? "" : "." + versionRange.substring(n + 1);
